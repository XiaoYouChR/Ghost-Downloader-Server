...
class DisplayIntent(BaseModel):
    ...
    key: str = Field(description="An i18n key for client-side localization, e.g., 'stage.description.merging_files'.")
...
class WorkerCapabilities(BaseModel):
    ...
    resumable: bool = Field(
        default=False,
        description="True if the worker supports pausing and resuming."
    )
...
class ConfigField(BaseModel):
    ...
    def check_consistency(self) -> 'ConfigField':
        if self.fieldType == 'choice':
            if not self.choices:
                raise ValueError("The 'choices' field is required when fieldType is 'choice'.")
            valid_values = [option.value for option in self.choices]
            if self.defaultValue not in valid_values:
                raise ValueError(f"defaultValue '{self.defaultValue}' is not a valid choice.")
        return self
