...
from sdk.ghost_downloader_sdk.interfaces import IParser
from sdk.ghost_downloader_sdk.models import StageDefinition, DisplayIntent

class HttpParser(IParser):
    """
    A generic parser that handles direct HTTP/HTTPS download links.
    It acts as a fallback when no other specialized parser can handle a URL.
    """
    ...
    def canHandle(self, url: str) -> bool:
        """Checks if the URL is a standard HTTP or HTTPS link."""
    ...
    async def getInitialStages(self, url: str, configOverrides: Dict[str, Any]) -> List[StageDefinition]:
        """
        Generates a single download stage for the given URL.
        """
            if not proxy:
        proxy = getProxy()
    ...
    return url, fileName, fileSize
        ...
        downloadStage = StageDefinition(
            displayIntent=DisplayIntent(
                key="stage.description.downloading.http",
                context={"filename": filename}
            ),
            workerType="download:http",
            payload={
                "url": url,
                # savePath 将由 CoreEngine 或用户设置决定，这里留空
                "savePath": None
            }
        )
